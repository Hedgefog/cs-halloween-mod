#pragma semicolon 1

#include <amxmodx>
#include <fakemeta>
#include <xs>

#include <api_custom_entities>

#include <hwn>
#include <hwn_wof>
#include <hwn_utils>

#define PLUGIN "[Hwn] Skeletons Horde WoF Spell"
#define AUTHOR "Hedgehog Fog"

#define SKELETON_EGG_ENTITY_NAME "hwn_skeleton_egg_big"
#define SKELETON_EGGS_NUM 5

new const g_szDetonateSound[] = "hwn/spells/spell_skeletons_horde_rise.wav";

new g_iGibsModelIndex;

new g_iWofSpell;

public plugin_precache() {
    g_iGibsModelIndex = precache_model("models/bonegibs.mdl");

    precache_sound(g_szDetonateSound);
}

public plugin_init() {
    register_plugin(PLUGIN, HWN_VERSION, AUTHOR);

    g_iWofSpell = Hwn_Wof_Spell_Register("Skeletons Horde");
}

public Hwn_Wof_Fw_Effect_Start(iSpell) {
    if (g_iWofSpell != iSpell) return;

    Hwn_Wof_Abort();

    new pDispenser = -1;
    while ((pDispenser = FindPumpkinDispenser(pDispenser)) != 0) {
        @Dispenser_SummonSkeletons(pDispenser);
    }
}

@Dispenser_SummonSkeletons(this) {
    static Float:vecOrigin[3]; pev(this, pev_origin, vecOrigin);

    static Float:vecSpawnOrigin[3];
    pev(this, pev_angles, vecSpawnOrigin);
    angle_vector(vecSpawnOrigin, ANGLEVECTOR_UP, vecSpawnOrigin);
    xs_vec_add_scaled(vecOrigin, vecSpawnOrigin, -64.0, vecSpawnOrigin);
    SpawnEggs(vecSpawnOrigin);

    static Float:vecGibsVelocity[3]; UTIL_RandomVector(-128.0, 128.0, vecGibsVelocity);
    UTIL_Message_BreakModel(vecOrigin, Float:{16.0, 16.0, 16.0}, vecGibsVelocity, 30, g_iGibsModelIndex, 20, 25, 0);

    UTIL_Message_Dlight(vecOrigin, 36, {HWN_COLOR_SECONDARY}, 30, 12);

    emit_sound(this, CHAN_BODY, g_szDetonateSound, VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
}

FindPumpkinDispenser(iStartIndex) {
    return engfunc(EngFunc_FindEntityByString, iStartIndex, "classname", "hwn_pumpkin_dispenser");
}

SpawnEggs(const Float:vecOrigin[3]) {
    for (new i = 0; i < SKELETON_EGGS_NUM; ++i) {
        new pEgg = CE_Create(SKELETON_EGG_ENTITY_NAME, vecOrigin);
        if (!pEgg) continue;

        dllfunc(DLLFunc_Spawn, pEgg);

        static Float:vecNewOrigin[3];
        UTIL_FindPlaceToTeleport(pEgg, vecOrigin, vecNewOrigin, HULL_HUMAN);
        set_pev(pEgg, pev_origin, vecNewOrigin);

        static Float:vecVelocity[3];
        xs_vec_set(vecVelocity, random_float(-96.0, 96.0), random_float(-96.0, 96.0), 128.0);
        set_pev(pEgg, pev_velocity, vecVelocity);
    }
}
